...
def _append_contextual_cta(base: str, *, enrollment_done: bool = False, during_form: bool = False, cta: str = "trial") -> str:
    """Добавляет следующий шаг, но не зовёт на демо повторно после оформленной заявки."""
    base = base.strip()
    ...
    if enrollment_done:
        if cta in {"trial", "schedule", "course", "price"}:
            return base + "\n\nМенеджер уже получил вашу заявку на демо и скоро свяжется с вами 🙂"
        return base

    ctas = {
        "course": "Лучше всего записаться на демо-презентацию, чтобы увидеть систему в действии. Хотите, я запишу вас? 🙂",
        "price": "Хотите, я запишу вас на демо-презентацию, чтобы менеджер подготовил индивидуальный расчет под ваши задачи? 🙂",
        "trial": "Хотите, я оформлю заявку на демо-презентацию? 🙂",
        "schedule": "Хотите, я запишу вас на демо, чтобы обсудить ваши требования и сроки внедрения? 🙂",
    }
    tail = ctas.get(cta, "")
    return base + ("\n\n" + tail if tail else "")
